...
<xsl:template match="baseO/OUser">
 ...
 <xsl:variable name="FlagWaiting"><xsl:value-of select="FlagWaiting"/></xsl:variable> 
  ...
  <title>Список Назначенных исследований (за период)</title>
 ...
 <div class="Section1">
    ...
    <p class="Cur14" align="center"><b>Назначенные исследований за период c <xsl:value-of select="Date1"/> по <xsl:value-of select="Date2"/></b></p>
    <p class="Cur12" align="left">
          <b>Отдел: </b><xsl:value-of select="Отдел"/>
          <xsl:if test='ГруппаНаборовТестов != ""'><b> / Группа наборов тестов: </b><xsl:value-of select="ГруппаНаборовТестов"/></xsl:if> 
  ...
  <table class="Cur10" border="1" sellspacing="0" width="100%" bordercolor="#000000" style="border-collapse: collapse; padding-left: 2px; padding-right: 2px;  
    ...
    <tr>
        ...
        <xsl:if test='$FlagEpis = 1'><td width="5%" align="center"><b>Лаб.номер</b></td></xsl:if>     
        <xsl:if test='$FlagFIO = 1'><td width="15%" align="center"><b>ФИО пациента</b></td></xsl:if>
        <xsl:if test='$FlagMRN = 1'><td width="5%" align="center"><b>№ МК</b></td></xsl:if>
        <xsl:if test='$FlagHospMRN = 1'><td width="5%" align="center"><b>Больнич.№ МК</b></td></xsl:if>
        <xsl:if test='$FlagHospEpis = 1'><td width="5%" align="center"><b>Больнич.№ Эпизода</b></td></xsl:if>                       
        ...
        <xsl:if test='$FlagTimeCol = 1'><td width="5%" align="center"><b>Время заб.</b></td></xsl:if>
        ...
        <xsl:if test='$FlagStorage = 1'><td width="10%" align="center"><b>Место хранения</b></td></xsl:if>
        <xsl:if test='$FlagDayLabNum = 1'><td width="10%" align="center"><b>Ежедн. номер</b></td></xsl:if>
        <td width="30%" align="center"><b>Список наборов тестов</b></td>
        <xsl:if test='$FlagWaiting = 0'>
            <td width="5%" align="center"><b>Кол-во назнач.</b></td>
            <td width="5%" align="center"><b>Кол-во вып.</b></td>
        </xsl:if>
    </tr>
    </thead>
 ...
 <br/>
    <!--общий итог -->
    <p class="CurGroup">
      <b>Всего за заданный период поступило образцов =  <xsl:value-of select="Itog-KolEpis"/>
         <br/>Количество назначенных исследований (наборов тестов) =   <xsl:value-of select="Itog-KolTS"/>
         <xsl:if test='$FlagWaiting = 0'> 
          <br/>Выполнено =   <xsl:value-of select="Itog-KolTS-A"/>
         </xsl:if>   
      </b>
    </p>
 <!-- подвал -->
