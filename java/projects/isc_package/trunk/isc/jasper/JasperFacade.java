...
public class JasperFacade {
  ...
  {
    JasperPrint jp = executeInternal(connString, fileName, params, "0");
    ByteArrayOutputStream out = new ByteArrayOutputStream();
    prepareExporter(jp, out, repFormat).exportReport();
    return out.toByteArray();
  }
  
  public static void executeAndSave(String connString, String fileName, String repFormat, Map params, String targetFileName, String debug) throws IOException, JRException, SQLException
  {
	  debug = (debug.equals(null)||(!debug.equals("1"))) ? "0" : "1"; // если 1, то переданные параметры с их значениями пишутся в файл c:\jasperlog.txt
	  
	  JasperPrint jp = executeInternal(connString, fileName, params, debug);
  ...
  private static JasperPrint executeInternal(String connString, String fileName, Map params, String debug) throws JRException, SQLException,IOException
  {
	debug = (debug.equals(null)||(!debug.equals("1"))) ? "0" : "1" ; // если 1, то переданные параметры с их значениями пишутся в файл c:\jasperlog.txt  
    ...
    {
         if (debug.equals("1")) {
        	  String filename="c:\\jasperlog.txt";
     	      // params
    	      Iterator<String> iterator = map.keySet().iterator();
			  String node;
			  while (iterator.hasNext()) {
			   		    node=iterator.next().toString();
			   		    logToFile(node.toString()+"="+map.get(node)+"\n", filename);
 			  } 
