...
public class ReportTest2 {
	...
	static String[] setTests = {"A010","B005"}; 
...
//	  Query
	  ...
	  //Details 
	    ...
	    for (int i=0; i<setTests.length;i++) {
	    	//add subreport to mainreport
		    JRDesignSubreport subReport = new JRDesignSubreport(styleProvider);
		    subReport.setY(i*100);
		    ...
		    //add parameter in mainreport for subreport
...
//		Fonts
	    ...
	    //Query
	    JRDesignQuery query = new JRDesignQuery();
	    query.setText("SELECT CTTSF_LabelText As Name,CTTSF_Reportable As Result, CTTSF_CL_Units As Units, CTTSF_CL_Text As Range from SQLUser.CT_TestSetFields where CT_TestSetFields.CTTSF_ParRef='A010'");
	...
	public static void main(String[] args) {
		...
		try
			...
			for (int i=0; i<setTests.length;i++) {
				...
				par = new String("sub"+i);
				params.put(par, subCompile);
			}
			JasperReport mainCompile = JasperCompileManager.compileReport(mainDesign);
			JasperPrint result = JasperFillManager.fillReport(mainCompile, params, connectionDB);
