...
//		  Title
	...
	public static void main(String[] args) {
		String aaa="jhgjhghjg";
		char c0=0;
		aaa=aaa+c0+"ggg";
		System.out.println(aaa.replace(new Character((char)0),','));
		String connString="jdbc:Cache://127.0.0.1:56773/DATA/?username=_SYSTEM&password=SYS";
		String setTest;
		HashMap params = new HashMap();
		int len = setTests.length;
		//String[] columns = {"setTest","subReport"};
		Vector columns = new Vector();
		columns.addElement("setTest");
		columns.addElement("subReport");
		Vector subReports = new Vector();
		...
		try
			...
			for (int i=0; i<len;i++) {
				...
				row.addElement(setTest);
				row.addElement(subCompile);
				subReports.addElement(row);
